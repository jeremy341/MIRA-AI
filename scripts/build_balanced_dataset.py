...
Sources:
  - SortWaste (official train/val/test splits; train is capped by balancing)
  - dmedhi (train/validation)
  - TACO (deterministic 70/15/15 split)
  - Roboflow (official train/valid/test splits)
  - SAM-labeled TrashNet (train/test)
...
SORTWASTE_MAP = {0: 3, 1: 3, 2: 3, 3: 3, 4: 3, 5: 2, 6: 1, 7: 3}
ROBOFLOW_MAP = {
    0: 1, 1: 1, 2: 1, 3: 4, 4: 0, 5: 1, 6: 4, 7: 3, 8: 2,
    9: 3, 10: 4, 11: 3, 12: 1, 13: 2, 14: 2, 15: 3, 16: 3,
    17: 1, 18: 4, 19: 3, 20: 0, 21: 0, 22: 0, 23: 0, 24: 2,
    25: 2, 26: 1, 27: 1, 28: 1, 29: 2, 30: 2, 31: 3, 32: 3,
    33: 3, 34: 3, 35: 3, 36: 2, 37: 2, 38: 2, 39: 2, 40: 2,
    41: 3, 42: 3, 43: 3, 44: 3, 45: 3, 46: 3, 47: 3, 48: 3,
    49: 1, 50: 4, 51: 1, 52: 4, 53: 3, 54: 3, 55: 3, 56: 3,
    57: 4, 58: 2, 59: 2, 60: 3, 61: 4, 62: 4, 63: 2,
}
TACO_MAP = {
    "Aluminium foil": 1, "Battery": 4, "Aluminium blister pack": 4,
    "Carded blister pack": 4, "Other plastic bottle": 3,
    "Clear plastic bottle": 3, "Glass bottle": 0, "Plastic bottle cap": 3,
    "Metal bottle cap": 1, "Broken glass": 0, "Food Can": 1,
    "Aerosol": 1, "Drink can": 1, "Toilet tube": 2, "Other carton": 2,
    "Egg carton": 2, "Drink carton": 2, "Corrugated carton": 2,
    "Meal carton": 2, "Pizza box": 2, "Paper cup": 2,
    "Disposable plastic cup": 3, "Foam cup": 3, "Glass cup": 0,
    "Other plastic cup": 3, "Food waste": 4, "Glass jar": 0,
    "Plastic lid": 3, "Metal lid": 1, "Other plastic": 3,
    "Magazine paper": 2, "Tissues": 2, "Wrapping paper": 2,
    "Normal paper": 2, "Paper bag": 2, "Plastified paper bag": 2,
    "Plastic film": 3, "Six pack rings": 3, "Garbage bag": 3,
    "Other plastic wrapper": 3, "Single-use carrier bag": 3,
    "Polypropylene bag": 3, "Crisp packet": 3, "Spread tub": 3,
    "Tupperware": 3, "Disposable food container": 3,
    "Foam food container": 3, "Other plastic container": 3,
    "Plastic gloves": 3, "Plastic glooves": 3, "Plastic utensils": 3,
...
def yolo_records(source: str, split: str, image_dir: Path, label_dir: Path,
                 mapping: dict[int, int] | None = None) -> list[Record]:
    records = []
    for image in sorted(image_dir.iterdir()):
        ...
        labels = remap_lines(lines, mapping) if mapping is not None else tuple(
            line for line in lines if len(line.split()) >= 5
        )
        if labels:
...
def coco_records(source: str, split: str, annotation_file: Path,
                 image_root: Path, name_map: dict[str, int]) -> list[Record]:
...
def load_taco() -> list[Record]:
    ...
    n = len(records)
    return [Record(r.source, "train" if i < n * 0.70 else "val" if i < n * 0.85 else "test",
                   r.source_id, r.image, r.labels) for i, r in enumerate(records)]
...
def load_dmedhi() -> list[Record]:
    ...
    records = []
    mapping = {"Glass": 0, "Metal": 1, "Paper": 2, "Cardboard": 2,
               "Plastic": 3, "Trash": 4, "Garbage": 4}
    for parquet_file, split in [("train-00000-of-00001.parquet", "train"),
...
def load_all_records() -> list[Record]:
    records = []
    sortwaste = DATASETS / "raw" / "sortwaste" / "dataset" / "dataset"
    sw_images = sortwaste / "splited_all_dataset"
    sw_labels = sortwaste / "splited_all_dataset_yolo"
    for split in ("train", "val", "test"):
        records.extend(yolo_records("sortwaste", split,
                                     sw_images / split / "images",
                                     sw_labels / split / "labels", SORTWASTE_MAP))
    ...
    for source_split, output_split in (("train", "train"), ("valid", "val"), ("test", "test")):
        records.extend(yolo_records("roboflow", output_split,
                                     roboflow / source_split / "images",
                                     roboflow / source_split / "labels", ROBOFLOW_MAP))

    trashnet = DATASETS / "trashnet_labeled"
    records.extend(yolo_records("trashnet", "train", trashnet / "images" / "train",
                                trashnet / "labels" / "train"))
    # The existing val split is kept as an independent tabletop test set.
    records.extend(yolo_records("trashnet", "test", trashnet / "images" / "val",
                                trashnet / "labels" / "val"))
    return records


def balance_training(records: list[Record]) -> tuple[list[Record], dict[int, int], int]:
    # SortWaste is valuable for camera geometry but is 95.9% plastic by box.
    # Do not let it determine the class balance. Add a small, low-plastic sample
    # after balancing the material-diverse sources.
    diverse = [record for record in records if record.source != "sortwaste"]
    sortwaste = [record for record in records if record.source == "sortwaste"]
    totals = Counter()
    ...
    counts = Counter()
    # Fill rare material classes first. Prefer single/few-object images so a
    # useful rare-class box does not bring a large cloud of plastic boxes with it.
    for class_id in (0, 4, 2, 1, 3):
        candidates = [
            record for record in diverse
            if class_id in record.counts and id(record) not in selected_by_id
        ]
    ...
    # Preserve some top-down SortWaste data without reintroducing its imbalance.
    geometry_candidates = [
        record for record in sortwaste if 1 in record.counts or 2 in record.counts
    ]
    geometry_candidates.sort(key=lambda record: (record.counts.get(3, 0), sum(record.counts.values())))
    selected.extend(geometry_candidates[:200])
    counts.update(counts_for_records(geometry_candidates[:200]))
    return selected, counts, target


def counts_for_records(records: list[Record]) -> Counter[int]:
    counts = Counter()
...
def write_dataset(records_by_split: dict[str, list[Record]], requested_train_counts: Counter,
                  target: int) -> None:
    if OUTPUT.exists():
    ...
    for split, records in records_by_split.items():
        for index, record in enumerate(records):
            ...
            manifest.append({"split": split, "source": record.source,
                             "source_split": record.split, "source_id": record.source_id,
                             "image": str(destination.relative_to(OUTPUT)),
                             "labels": list(record.labels), "sha256": digest})
            copied[split] += 1
    ...
    (OUTPUT / "dataset.yaml").write_text(
        f"train: {OUTPUT / 'images' / 'train'}\n"
        f"val: {OUTPUT / 'images' / 'val'}\n"
        f"test: {OUTPUT / 'images' / 'test'}\n"
        "nc: 5\n"
        f"names: {CLASSES}\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    train_candidates = [record for record in records if record.split == "train"]
    ...
    print("Selected training sources:", dict(Counter(record.source for record in selected)))
