...
def plot_det_map():
    ...
    for bar, _val in zip(bars, map50, strict=True):
        height = bar.get_height()
        ax.annotate(
            f"{height:.1f}%",
            xy=(bar.get_x() + bar.get_width() / 2, height),
...
def plot_heatmap():
    ...
    experiments = ["EXP-013 (TACO+TrashNet)", "EXP-014 (+Roboflow)", "EXP-015 (+WaRP)", "EXP-016 (WaRP-only)"]

    # mAP50 values
    # NOTE: Hardcoded per-class mAP50 metrics. Update when new experiments are run. Ideally sourced from results/*.csv.
    data = np.array(
        [
...
def plot_ema_filter_simulation():
    ...
    ax.plot(
        raw_signal,
        label="Rohdaten (Modellvorhersage mit Jitter)",
        color="#e31a1c",
        alpha=0.6,
        linewidth=1.5,
        linestyle=":",
