...
# Add src/ to path so camera_service can import config
...
from models import (
    CameraConfig, ModelConfig, SystemStatus, StatusUpdate,
...
async def load_model(model_config: ModelConfig):
    ...
    success = await camera_service.load_model(
        model_config.name,
        model_config
    )
    ...
    if success:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to load model {model_config.name}"
        )
